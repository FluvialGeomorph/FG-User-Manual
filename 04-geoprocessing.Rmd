...
The purpose of this step is to convert the `banks_xxx` polygon created in one of the previous steps into a feature class that has two records, one representing the left descending bankline and another the right descending bankline for the stream reach. 

* Use the `Polygon To Line` tool to convert the `banks_xxx` polygon feature class created in one of the previous steps into a line feature class named `banklines`. 
* Add a new string field to the `banklines` feature class named `ReachName`.  
...
* Use the `Explode Multipart Feature` on the advanced editing toolbar to explode the multipart line feature. 
...
* The `flowline` feature class will be generalized to create the `valleyline` feature class. This will be an iterative process of generating a new smoothed line and comparing it with the `loop_points` feature class. The goal is to find the smooth line that passses through the defined meander loops. 
* Use the `Smooth Line` tool to smooth the `flowline` feature class. Use the `PAEK` smoothing algorithm. Start with a smoothing tolarance value of 100 meters. Name the output feature class `valleyline_100`
* Check how well the new smoothed `valleyline_100` feature class passes through the meander loops defined in `loop_points`. If the fit is unacceptable, try increasing the smoothing tolerance value. 
...
* The ideal smoothed valleyline should: 
    ...
    * Cross the flowline at the inflection points between meander loops (ideal)
* Select the smoothed line with the best fit. Delete the unselected smoothed lines. Rename the selected smoothed line `valleyline`. 
