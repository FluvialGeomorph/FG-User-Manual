...
* Complete tis step for both the `riffle_channel` and `riffle_floodplain` feature classes. 
...
* When the tool completes, check that this tools runs correctly by ensuring that the `loop`, `bend`, `position`, `valley_POINT_X`, `valley_POINT_Y`, and `valley_POINT_M` fields are populated. Only bankline point features whithin loops and bends will have values in the `loop`, `bend`, and `position` fields. However, all records should have values in the `valley_POINT_X`, `valley_POINT_Y`, and `valley_POINT_M` fields. 
...
* Check that the `loop` and `bend` fileds are assigned the appropriate loop and bend. 
* Complete this step for both the `riffle_channel` and `riffle_floodplain` feature classes. 
