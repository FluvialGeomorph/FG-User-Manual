...
2.   Open the ESRI `🛠️ Smooth Line tool` to smooth the reach flowline feature class from the most recent year as the input feature.
...
8. Choose Final `🗺️ Valleyline`
9. Add the `🗺️ floodplain polygon` (higher bank_raw_###) and all of the candidate `🗺️ valleyline` feature classes to the current map.
10. Identify the `🗺️ valleyline` feature class that has the highest smoothing tolerance value that also is mostly contained within the extent of the `🗺️ floodplain polygon`.
...
1. Add the `🗺️ valleyline` layer as the input.
2. Add the output multi-ring buffer to the newest year gdb. Name it `🗺️ multiring_buffer`.
...
1. Create a new point feature class named `🗺️ loop_points`. `📊 Enable` `🧮 z and m` values. The feature class should contain the following fields:
...
1. Open the `🛠️ Bankline Points tool` to assign loops, bends, elevations and valley line positions to the `🗺️ bankline_points`.
2. Add the feature dataset for the most recent year.
3. Add the `🗺️ loop point`, `🗺️ banklines`, and `🗺️ valleyline` layers from the most recent year.
4. Add the `🗺️ demhydro` for the mosts recent year.
...
1. Open the `🛠️ Assign Loops tool` to assign loops and bends to a `🗺️ cross_section` feature class.
2. Add the `🗺️ cross_section` feature class from the the most recent survey event dataset.
3. Add the `🗺️ bankline_points` to the most recent survey event dataset
...
The purpose of this step is to calculate the L3 dimensions for the regularly spaced and `🗺️ riffle cross_section` feature classes for each reach.
...
2. Add the `🗺️ level 2 XS_Dimensions` from the same survey event to `🗺️ xs_dimesions`.
3. Add  the `🗺️ bankline_points` from the same survey event to `🗺️ bankline_points`.
...
3. Add the `🗺️ level 2 XS_Dimensions` from the same survey event to `📊 fc`.
4. Seclet the `🧮 Seq` in the dropdown for the `📊 fc_field`.
5. Add the `🗺️ level 3 dimension csv` to the `📊 csv_file`.
...
2. Add the feature dataset for the current survey event.
3. Add the `🗺️ xs_dimensions_l3` to `📊 xs_dims`.
...
4. Add the `🗺️ cross_section` feature class to `📊 xs_fc`.
...
10. Add the `🗺️ features` feature class for the site to the `📊 features_fc`.
