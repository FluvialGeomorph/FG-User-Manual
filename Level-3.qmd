...
2.   Open the ESRI `🛠️ Smooth Line tool` to smooth the reach `🗺️ flowline` feature class from the most recent year as the input feature.
...
6. Click Run. 
    ...
    ii. Depending on the detail of the `🗺️ flowline` feature class geometry, you may need to select a different range or set of step values of the `📊 Smoothing Tolerance` parameter values to evaluate.
...
8. Choose Final `🗺️ Valleyline` feature class.
9. Add the `🗺️ floodplain polygon` (higher bank_raw_###) and all of the candidate `🗺️ valleyline` feature classes to the current map.
10. Identify the `🗺️ valleyline` feature class that has the highest smoothing tolerance value that also is mostly contained within the extent of the `🗺️ floodplain polygon` feature class.
...
1. Add the `🗺️ valleyline` feature class as the input.
2. Add the output multi-ring buffer to the newest year gdb. Name it `🗺️ multiring_buffer` feature class.
...
1. Create a new point feature class named `🗺️ loop_points` feature class. `📊 Enable` `🧮 z and m` values. The feature class should contain the following fields:
   ...
   iv. `📊 position`: `🧮 Text (10)` - The purpose of this field is to store position identifier for the point. This field must have one three values: `🧮start`, `🧮end`, or `🧮apex`.
...
1. `🗺️ Loop_points` features are used to define the start and end location of loop bends and the location of a loop’s apex.
2. `🗺️ Loop_points` features are always placed along (i.e., snapped to) an outer bankline of the feature.
...
#### Multiple Reaches:
  ...
  - Set the loop field value for each upstream reach to the upstream-most value (i.e., the highest loop value of the downstream reach’s `🗺️ loop_points` feature class) of the downstream reach. For example, set the value of the field loop of the first Reach-2 loop_points feature to 8 if the maximum value of Reach-1’s loop_points feature class loop field is 7.
...
1. Open the `🛠️ Bankline Points tool` to assign loops, bends, elevations and valley line positions to the `🗺️ bankline_points` feature class.
2. Add the feature dataset for the most recent year.
3. Add the `🗺️ loop point`, `🗺️ banklines`, and `🗺️ valleyline` feature classes from the most recent year.
4. Add the `🗺️ demhydro` raster for the mosts recent year.
5. Set the `📊 station_distance` at `🧮 1`.
6. Hit Run.
7. In the `🧰 Check toolset` run the `🛠️ Check Bankline Points tool`.
...
8. Apply the following definition query to the `🗺️ bankline_points` feature class: “loop IS NOT NULL”. Only bankline_points features between loop_points features should be visible.
...
1. Open the `🛠️ Assign Loops tool` to assign loops and bends to a `🗺️ XS` line feature class.
2. Add the `🗺️ cross_section` feature class from the the most recent survey event dataset.
3. Add the `🗺️ bankline_points` feature class to the most recent survey event dataset
...
4. __Repeat__ this step for both the `🗺️ riffle_channel` and ` 🗺️riffle_floodplain` feature classes.  
...
The purpose of this step is to calculate the L3 dimensions for the regularly spaced and `🗺️ riffle _XS` feature classes for each reach.
...
1. Open the `🛠️ XS Planform, Level 3` tool to calculate the L3 dimensions.
2. Add the `🗺️ level 2 XS_Dimensions` feature class from the same survey event to `🗺️ xs_dimesions`.
3. Add  the `🗺️ bankline_points` feature class from the same survey event to `🗺️ bankline_points` feature class.
4. Refresh the Survey event year folder to update with the `📈 XS-Planform Level 3 csv`.
5. Verify in the attribute table that the L3 dimensions have been calculated

### Join from CSV (Data Management toolbox)

1. Open the `🛠️ Join from CSV` tool in the ` 🧰 Data Management` toolbox.
2. Add the feature dataset for the current survey event.
3. Add the `🗺️ level 2 XS_Dimensions` feature class from the same survey event to `📊 fc`.
4. Select the `🧮 Seq` in the dropdown for the `📊 fc_field`.
5. Add the `📈 level 3 dimension csv` spreadsheet to the `📊 csv_file`.
...
1. Open the `🛠️ RAS Watersurface tool`.
2. Add the feature dataset for the current survey event.
3. Add the `🗺️ xs_dimensions_l3` feature class to `📊 xs_dims`.
4. Add a `🗺️ ras HEC model` raster containing water surface elevation field.
...
1. Open the `🛠️ Level 3 Report tool` to produce the Level 3 Report.
2. Add the value of the `📊 ReachName` field used in the `🗺️ flowline` feature class to `📊 stream`.
3. Add the `🗺️ flowline` feature class for the base year survey to `📊 flowline_fc`.
4. Add the `🗺️ XS` line feature class to `📊 xs_fc`.
5. Add the  `🗺️ *_dims_L3` feature class calculated for the regular cross sections of the most recent survey event.
6. Add the `🗺️ xs_points_*` feature class for the most recent survey event first (i.e., the base year) and then the previous surveys in reverse chronological order (e.g., 2016, 2010, 2006).
...
8. Add the `🗺️ DEM` raster for the most recent survey event.
9. Add the `🗺️ banklines` feature class created for the most recent survey event.
10. Add the `🗺️ features` point feature class for the site to the `📊 features_fc`.
11. Enter the same `🧮 bf_estimate` from level 2 report into the `📊 bf_estimate` box.
