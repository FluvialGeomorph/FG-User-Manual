---
title: "Level-1 Initial Channel Report"
format: html
---


{{< badge beginner >}} {{< badge evaluate >}}
...
The `🗺️ flowline` feature class extends from the downstream end of the reach to the upstream end. Cross sections should be spaced regularly across the entire reach, without any gaps at the upper or lower end.  
...
This report section contains two longitudinal profile graphs, one drawn using the `🗺️ flowline_points` feature class, and the second is drawn using the cross section dimensions,  `🗺️ *_dims_L1` feature class.  

### Elevation values are reasonable
The elevation values displayed on the map or graph should be reasonable. Use the ESRI `🛠️ Explore` tool to check the DEM raster `🧮 elevation` values. By convention in FluvialGeomorph, DEM values must always have vertical units = feet. 
...
### Multiple surveys are correctly calibrated to the base event
...
If all of the surveys' last station values do not align, it means that the previous surveys were not correctly calibrated to the base event flowline. Previous events surveys will need to have the `🛠️ Flowline Points` tool rerun using the base event survey's `🗺️ flowline_points` feature class for calibration. 
...
Longitudinal graphs are labeled using the rows in the `🗺️ features` feature class. Ensure these river features are labeled correctly and are located at the correct longitudinal position. 


## Cross Section Metrics {#L1_xs_metrics}
This report section contains a Level 1 cross section metrics plot drawn using the `🗺️ XS_dimensions` feature class, `🗺️ *_dims_L1`. 

### Cross section watershed area calculated correctly
The watershed area metric is calculated by the `🛠️ XS Watershed Area` tool. Given the complexities of automatically calculating watershed area for high resolution stream features, some watershed areas may not have been calculated correctly. Rerunning the `🛠️ XS Watershed Area` tool or manual adjustment of the `📊 Watershed_Area_SqMile` field may be required.  

* Watershed values are reasonable. 
* Watershed area must increase moving downstream. 

### Elevation smoothing is acceptable
High resolution LiDAR derived stream elevations can be noisy. Smoothing the elevations longitudinally is a way reduce noise. However, too much smoothing can be applied, eliminating any of the relationships we want to observe. By visually comparing the raw elevation and the smoothed elevation graphs, verify that the amount of smoothing sufficiently reduces noise, but does not not go too far and removes substantive variation. Smoothing is controlled in the `🛠️ XS Dimensions` tools using the `📊 loess_span` parameter. 

### Ensure metrics values are smooth
Many reach metrics are calculated using a longitudinal moving window analysis. The size of the moving window is controlled in the `🛠️ XS Dimensions` tools using the `📊 lead_n` parameter. A smaller value applies a narrower moving window and a larger value applies a wider moving window. If the moving window is too small, metric values will be highly variable along the stream. As the moving window size gets larger, metric value variation will be reduced. 
...
This report section contains a cross section map and a cross section profile graph for each cross section in the reach. The map is drawn from the cross section dimensions `🗺️ *_dims_L1` feature class and the `🗺️ dem_hydro` raster. The cross section profile graph is drawn using the cross section points `🗺️ *_points_L1` feature classes for each survey.  
...
` 🗺️ XS` line feature class is generated based on the flowline which may zigzag within the channel causing the `🗺️ XS` lines to be angled across the banklines. Edit the `🗺️ XS` lines to be perpendicular across both banklines.

### Cross section (XS) covers the channel extent
Viewing the `🗺️ XS` line feature class with the `🗺️ hydro_DEM` raster displayed with DRA at about 1:1000 scale shows a clear picture of the channel. Using the `🗺️ REM` raster use the banks raw feature to extend the `🗺️ XS` lines across the determined banklines. Be sure the `🗺️ XS` lines covers both banks.

### Cross section extends onto floodplain
Using the `🗺️ REM` raster determined floodplain feature, extend the `🗺️ XS` lines to cover the entire floodplain at its location.

### Cross section not located on built infrastructure
Using the `🗺️ hydro_DEM` raster and aerial imagery place the `🗺️ XS` lines away from bridges and river crossing that have artificial heights. Place the `🗺️ XS` lines where it will determine the bank elevations.

### Cross section drawn within the DEM extent
The `🗺️ hydro_DEM` raster and `🗺️ REM` raster should be built at the floodplain extent. Edit the `🗺️ XS` lines to cover the floodplain and remain within the limits of the raster to extract the elevation and avoid Null values.

### Cross section y-axis range is not too large
Edit the `🗺️ XS` lines just beyond the floodplain extent, but not too far up the cliff or hill to create a major change in elevation. Allow the `🗺️ XS` lines to display the bank elevation data clearly.

### Cross section x-axis range is not too large
Where possible keep a consistent `🗺️ XS` lines length to allow the viewer to see multiple `🗺️ XS` lines of similar size in order to see channel and bank changes.

## Next Steps
{{< badge reference >}}

### Report 
1. Rerun  Level 1 Report as needed.
2. Re-review the rerun of Level 1 Report.
