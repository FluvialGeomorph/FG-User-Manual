...
1. Copy the most recent years `🗺️ cross section (XS)` as feature class named `🗺️ riffle_floodplain`. 
...
### 	Calculate Cross Section Watershed Area 
 ...
 3.	Add the `🗺️ feature` dataset.
 4.	Add the extended `🗺️ XS line layer.
 5.	Add the `🗺️ flowline.
 ...
 8.	Hit Run.
    ...
    ii.	The result is a new attribute field in the `🗺️ XS` feature class that has a calculated watershed area for each regularly spaced cross section.
...
### 	XS River Position 
 ...
 2.	Enter `🗺️ feature` dataset for more recent geodatabase.
 3.	Input extended `🗺️ XS`.
 4.	Input `🗺️ Flowline Points`.
 5.	Hit Run. Complete for both the `🗺️ channel_XS` and `🗺️ floodplain_XS` 
...
2.	Enter `🗺️ feature` dataset for more recent geodatabase.
3.	Input extended `🗺️ Cross Sections`.
4.	Input `🗺️ dem_hydro`.
5.	Change `📊 dem_units` to `🧮 ft`.
6.	Input `🗺️ REM` for the detrend_dem.
7.	`📊 Station Distance` is `🧮 1`.
8.	Hit Run. Complete for both the `🗺️ channel_XS` and `🗺️ floodplain_XS` .
...
2.	Enter `🗺️ feature` dataset for the most recent year.
3.	Input extended `🗺️ XS points`.
4.	Input `🗺️ channel polygon`.
5.	Input `🗺️ floodplain polygon`.
6.	Leave `📊 buffer distance` as the `🧮 default`.
7.	Repeat for earlier year extended `🗺️ XS Points`.

### 	Create Banklines 

1.	Turn `🗺️ channel bank_raw` polygon layer into a line layer. 
2.	Use Esri `🛠️ Poygon to Line` tool.
3.	Add the `🗺️ bank_raw channel` polygon as the input feature.
4.	Add the `🗺️ feature` dataset for the more recent gdb.
5.	Hit Run.
...
2.	Extended `🗺️ XS’s` for the newest year are added into xs_line fc.
...
2.	`🗺️ XS’s` dimension level 1 for the riffles into extended xs_line fc. 
3.	`🗺️ XS’s_points for the channel riffles are added to the extended XS_point_fc. 
4.	Bankfull_elevation is from the channel depth of the `🗺️ bank_raw_channel` layer. Ex. 103.5
...
### 	Join from CSV 
1.	In the FluvialGeomorph toolbox, open `🛠️ Join From CSV` located in the `🧰 Data Management` toolset.
2.	Feature_dataset will be the `🗺️ features` dataset of the most recent gdb.
3.	Fc will be the extended `🗺️ XS _layout_dims_L1` line feature class.
4.	Choose `🧮 Seq` in the dropdown for the `📊 fc_field`.
5.	The csv_file will be the `🗺️ .csv` file created from the `🛠️ XS Dimensions Level 2` Tool.
6.	`📊 Csv_field` will match the `📊 fc_field` so it should also be `🧮 Seq`.
7.	Hit Run.
...
4.	`📊 Extended Xs_fc` will be the `🗺️ XS` feature class for the most recent survey event.
5.	`📊 Extended Xs_dimensions_fc` will be the line feature class created during the `🛠️ Join From CSV` tool.
6.	`📊 Extended Xs_points_1` will be the `🗺️ XS` points point feature class created from the XS Points tool. Use the data from the most recent survey event.
    i.	Extended Xs_points_2 will be the `🗺️ XS` points point feature class for the older event.
...
11.	`📊 Dem` will need the `🗺️ dem_hydro` from the most recent survey event.
