...
Level 2 is to estimate the REMed bankfull elevation for the base year for each reach. This is accomplished by identifying and mapping of all cross sections and roughly estimate channel bankfull dimensions and profile for each reach.
...
1. Copy the most recent years cross section `🗺️ XS` as line feature class named `🗺️ riffle_floodplain` feature class. 
    i. You'll use the original XS lines that represent the rock riffles and delete all others.
    ii. extend each XS line high ground to high ground of the dem_hydro outside the floodplain on each side. XS extent must be beyond the floodplain at minimum and kept perpendicular to the channel.
    iii. This provides a feature class already with correct projection and attribute fields.
...
1. Digitize riffle cross sections beginning with the left descending bank. While editing, use the `Reverse Direction` command (aka flip) to ensure riffles are digitized in the correct direction. 
   i. A red vertex denotes the end of a line segment. Therefore, the red end vertex should be on the right descending bank. 
   ii. Check that each cross section is digitized in the correct direction (start at the left descending bank and end on the right descending bank) before going on to the next step. 
   iii. Ensure that riffle cross sections are digitized to the full width of the active floodplain. Edit each feature to ensure that it extends at least to the edge of the floodplain boundary layer, but no further. 
...
2. The downstream-most cross section in the site should be numbered starting with the Seq field value of 1 and increase moving upstream. 
    i. If necessary, use the tool to set the starting `📊 Seq` value for each reach. 


### 	Calculate Cross Section Watershed Area 
 1.	Go to FluvialGeomorph toolbox open the `🛠️ `XS Wastershed Area tool.
 2.	Start with the more recent data first.
 3.	Add the `🗺️ features` point feature class.
 4.	Add the extended `🗺️ XS` line feature class.
 5.	Add the `🗺️ flowline` feature class.
 ...
 9.	Once it is done running, open attribute table and see if anything in the `📊 watershedareaSqMile` is off.
    i.	Some of the cross sections `📊 Watershed_Area_SqMile` may not follow the trend. If this is the case, manually enter in the values to fill in the gaps. If most of the cross sections are missing these or display incorrect values, rerun the Watershed area tool and increase the snap distance.
...
### 	XS River Position 
 ...
 2.	Enter `🗺️ features` point feature class for more recent geodatabase.
 3.	Input extended `🗺️ XS` line feature class.
 4.	Input `🗺️ flowlinepoints` feature class.
 5.	Hit Run. Complete for both the `🗺️ channel_XS` and `🗺️ floodplain_XS` feature classes. 
 6.	The result adds new fields in the attribute table of the `🗺️ XS`line feature class. 
 7.	Two temporary charts will populate the Contents tab under the `🗺️ XS` line feature class (`📊 XS Seq by 📊 km_to_mouth, 📊 XS Seq by 📊 Watershed Area sq mile`). If the charts are removed from the contents pane, you will need to run the `🛠️ XS River Position` Tool to view them again.
...
8.	Hit Run. Complete for both the `🗺️ channel_XS` and `🗺️ floodplain_XS` point feature classes.
...
2.	Enter `🗺️ features` point feature class for the most recent year.
3.	Input extended `🗺️ XSpoints` feature class.
4.	Input `🗺️ channel polygon` feature class.
5.	Input `🗺️ floodplain polygon` feature class.
6.	Leave `📊 buffer distance` as the `🧮 default`.
7.	Repeat for earlier year extended `🗺️ XSpoints` feature class.

### 	Create Banklines 

1.	Turn `🗺️ channel bank_raw` polygon feature class into a line layer. 
2.	Use Esri `🛠️ Poygon to Line` tool.
3.	Add the `🗺️ bank_raw channel` polygon feature class as the input feature.
4.	Add the `🗺️ features` point feature class for the more recent gdb.
5.	Hit Run.
...
2.	Extended `🗺️ XS` line feature class for the newest year are added into xs_line fc.
...
1.	In the FluvialGeomorph toolbox, open `🛠️ Join From CSV` located in the Data Management toolset.
2.	`🗺️ features` point feature class will be the feature dataset of the most recent gdb.
3.	Fc will be the `🗺️ XS` line feature class.
4.	Choose `🧮 Seq` in the dropdown for the `📊 fc_field`.
5.	The csv_file will be the .csv file created from the `🛠️ XS Dimensions Level 1` Tool.
6.	`📊 Csv_field` will match the `📊 fc_field` so it should also be `🧮 Seq`.
7.	Hit Run.
8.	A new line feature class ending with `🗺️ *dims_L1` will be added to the feature dataset.
...
::: {style="float: none; margin:1px 30px; width: 230px;"}
...
### 	L2 XS Dimensions 
1.	In the FluvialGeomorph toolbox, open `🛠️ XS Dimensions, Level 2`.
2.	`🗺️ XS` dimension level 1 line feature class for the riffles into extended `📊 xs_line fc`. 
3.	`🗺️ XSpoints` feature class for the channel riffles are added to the extended `📊 XS_point_fc`. 
4.	Bankfull_elevation is from the channel depth of the `🗺️ bank_raw_channel` feature class. Ex. 103.5
...
### 	Join from CSV 
1.	In the FluvialGeomorph toolbox, open `🛠️ Join From CSV` located in the `🧰 Data Management` toolset.
2.	Feature_dataset will be the `🗺️ features` point feature class of the most recent gdb.
3.	Fc will be the extended `🗺️ XS_layout_dims_L1` line feature class.
4.	Choose `🧮 Seq` in the dropdown for the `📊 fc_field`.
5.	The csv_file will be the `📈 .csv` spreadsheet file created from the `🛠️ XS Dimensions Level 2` Tool.
6.	`📊 Csv_field` will match the `📊 fc_field` so it should also be `🧮 Seq`.
7.	Hit Run.
8.	A new line feature class ending with `🗺️ *dims_L2` line feature class will be added to the feature dataset.

### 	Generate Level 2 Report
1.	In the FluvialGeomorph toolbox, open the `🛠️ Report – L2` located in the `🧰 Reports` toolset.
...
3.	`📊 Flowline_fc` will be the `🗺️ flowline` feature class from the recent survey event.
4.	`📊 Extended Xs_fc` will be the `🗺️ XS` line feature class for the most recent survey event.
5.	`📊 Extended Xs_dimensions_fc` will be the line feature class created during the `🛠️ Join From CSV` tool.
6.	`📊 Extended Xs_points_1` will be the `🗺️ XSpoints` feature class created from the XS Points tool. Use the data from the most recent survey event.
    i.	Extended Xs_points_2 will be the `🗺️ XSpoints` feature class for the older event.
...
11.	`📊 Dem` will need the `🗺️ dem_hydro` raster from the most recent survey event.
