...
1.	Get the latest version of the toolbox from this page under Releases. 
     [ArcGIS Toolbox](https://github.com/FluvialGeomorph/FluvialGeomorph-toolbox)
2.	Place the install in a location that all required applications can access.
3.	In ArcCatalog open the Fluvgeo toolbox and under install 
      **click Install R Package box and Hit run**.

::: {.callout-note}
For more detailed installation information refer to FluvialGeomorph [Tech Manual](https://fluvialgeomorph.github.io/FG-Tech-Manual/2-1-Install.html)   
:::
...
1.	Add `🧰 LAStools toolbox` to ArcGISPro. Open the `🧰 LAStools toolbox` and select `🛠️ las2lasPro` (transform) tool.
...
1.	Open LAStools `🛠️ las2lasPro` (transform) tool
...
<!--Left align need additional margin space to create space for the outline  numbers -->
...
1.	In the geoprocessing tab search `🛠️ Create LAS Dataset` tool.
...
4.	Under `📊 Surface Constraints` the input feature is `🗺️ Boundary`.
...
1.	In the geoprocessing tab search `🛠️ LAS Dataset to Raster` tool.
...
2.	Use the `🛠️ measure tool` to measure the length of pixel. Should be `🧮 1 ft`.
...
3.	For the features line feature class `📊 Name`, name is `🧮Site#_EVENT. Ex. Site263_2008`.
...
7.	Name it `🗺️ cutlines` feature class, Change feature `📊 class type` to `🧮 line`, and finish.
8.	Identify Flow Blockages 
    i.	Go to the edit ribbon and select `🛠️ create feature` and select cutlines
    ...
    /* Reduce space between image and caption inside columns */
...
### 	Hydro Modify DEM
1.	In the catalog pane, go to the `FluvialGeomorph toolbox`. Expand tools and select `🛠️ Hydro DEM` tool.
2.	For output_workspace, drag your geodatabase from the catalog pane (Created prior) into the space.
3.	For `📊 cutlines`, drag your `🗺️ cutline` feature classes into the space.
4.	For `📊 dem`, drag your `🗺️ hydro_DEM` raster dataset into the space. 
...
1.	Open `🛠️ Derive Stream as Line` tool 
2.	Select the `🗺️ dem_hydro` raster as the input surface raster
...
1.	Open `🛠️ Smooth Line` tool 
2.  Set `📊 smoothing` between `🧮 2 and 15` depending on the size and flow of the channel.
...
### 	Create Flowline 
1.	In the FluvialGeomorph toolbox, select the `🛠️ Flowline` tool.
...
5.	The output is a line feature class called `🗺️ flowline` feature class. This tool derives the site flowline and smooths the stream_network geometry and converts the flowline into a route.
...
1.	In the FluvialGeomorph toolbox, select the `🛠️ Flowlinepoints` tool.
...
3.	The dem will be the `🗺️ dem_hydro` raster. 
...
h.	Hit run. 
i.	Note: Before you run the `🛠️ flowline_points` tool for the older dataset, make sure the newer `🛠️ Flowlinepoints` tool is done running. 
i.	For the older dataset, fill in the information like when running the tool for the newer data. 
j.	For calibration points, drag the `🗺️ flowline_points` feature class created from the newer data.
...
m.	Hit run. 
i.	The result converts the flowline into a series of points along the reach. The tool converts the `🗺️ flowline` feature class into a route, calculates the distance to the mouth of the river for all vertices, and creates a `🗺️ flowline_points` feature class.

### 	Create Features
1.	Right-click on the feature dataset and click `🛠️ Create Feature Class` tool and select point for the geometry type and place in either year feature dataset.
2.	Add fields for `📊 Name` `🧮 (text)` and `📊 kmto_mouth ` `🧮 (double)`. 
3.	Drag the `🗺️ features` point feature class into the map. Move it to first in drawing order. 
4.	Turn off all layers except the flowline points, so you can see the basemap.
5.	On the ribbon, click edit and `🛠️ Create Features` tool. 
...
8.	Open attribute table for `🗺️ features` point feature class. Under km_to_mouth put the number that was correlated with “Measure”. 
...
a.	Right click on the `🗺️ features` point feature class for most recent data. Go to import and select Feature Class(es).
b.	For the input features, go to a previous county folder and expand the feature dataset for the most recent geodatabase under any Site# and import that database layer called `🗺️ features` point feature class. 
...
### 	REM 
1.	In the FluvialGeomorph toolbox, select the `🛠️ REM` tool.
2.	Fill in the parameters with the corresponding data. Make sure for dem you choose the `🗺️ hydro_dem` raster. 
...
1.	Have the `🗺️ flowline` feature class, `🗺️ demhydro` raster and `🗺️ REM` raster for the newer year. 
2.	Find the elevation in the `🗺️ REM` raster for the bank and the `🗺️ flowline` feature class. Check multiple locations along the stream, including the site location. 
3.	Find the average difference in elevation between the bank and the flowline.  
4.	Subtract the two REM values to get the bankfull depth for the channel. Ex. bank elevation 104 and flowline elevation 100 then the channel depth is 104. This value is used in the `🛠️ Water Surface Extent` tool as the `📊 REM_value`. This will become the channel bank_raw value.
5.	Next Multiply the last number of the channel bank_raw value by 2 and use it as the `📊 REM_value` for the `🗺️ floodplain bank_raw` feature class. Ex. channel bank_raw 104 make the floodplain bank_raw 108. 

### 	Create Water Surface Extent  
1.	In the FluvialGeomorph toolbox, select the `🛠️ Water Surface Extent` tool. 
2.	Fill in the `🗺️ features` point feature class and the `🗺️ REM` raster for from the most recent year.
...
::: {style="float:right; margin: 5px; width: 230px;"}
![Sample of a channel bank and floodplain layer](images/channel.jpg){height="2.3in"}
:::

1.	Open the attribute table for the `🗺️ channelbanks_raw` feature class. Sort the Shape_Area column descending.
...
3.	The `🗺️ channelbanks_raw` feature class need to be trimmed of any incoming confluences. Then save the edits.
4.	Repeat Create `🛠️ Water Surface Extent` tool for the `🗺️ floodplainbanks_raw` feature class. Leave an additional area or confluences. 

### 	Create Cross Sections(XS) (only for newest dataset) 
1.	In the FluvialGeomorph toolbox, select the `🛠️ XS Layout` tool. 
2.	Fill in the `🗺️ features` point feature class and `🗺️ flowline` feature class from the most recent geodatabase.
...
1.	Run `🛠️ Resequence` only if any cross sections were deleted in the adjustment process.

### 	Calculate XS Watershed Area (only for newest dataset)
1.	In the fluvialgeomorph folder, expand layers and add a clip of the needed county from the NHDPlusFac_National.tif. This dataset can be found at [EPA](https://www.epa.gov/waterdata/get-nhdplus-national-hydrography-dataset-plus-data#DownloadHR)

::: {.callout-tip}
  Having countywide `🗺️ NHD` raster layer improves processing time if there is more than one site in a county.
...
4.	Clipping geometry is displaying current extent. Make sure you are zoomed out to the whole county, so each site is located within extent.
    i.	Optional: Create a clip for each project site of the `🗺️ NHDPlusFac_National.tif`raster with the extent zoomed out 1:50000 centered on the site.
5.	Go to FluvialGeomorph toolbox. Add `🛠️ Wastershed Area` tool.
...
8.	Hit Run.
    ...
    ii.	The result is a new attribute field in the `🗺️ XS` line feature class that has a calculated watershed area for each regularly spaced cross section.
9.	Once it is done running, open attribute table and see if anything in the watershedareaSqMile is off.
    i.	Some of the `🗺️ XS` line feature class `📊 Watershed_Area_SqMile` may not follow the trend. If this is the case, manually enter in the values to fill in the gaps. If most of the XS's are missing these or display incorrect values, rerun `🛠️ Watershed Area` tool and increase the `📊 snap distance`.
...
10.	Import the finished `🗺️ XS_300_100` line feature classto the earlier year feature dataset. 

### 	XS River Position 
1.	In the FluvialGeomorph toolbox, select `🛠️ XS River Position`.
2.	Enter `🗺️ features` point feature class from more recent geodatabase.
3.	Input `🗺️ XS` line feature class.
4.	Input `🗺️ flowlinepoints` feature class.
5.	Hit Run.
6.	The result adds new fields in the attribute table of the `🗺️ XS` line feature class. 
7.	Two temporary charts will populate the Contents tab under the `🗺️ XS` line feature Cclass (`🧮 XS Seq` by `📊 km_to_mouth`, `🧮 XS Seq` by `📊 Watershed Area sq mile`). If the charts are removed from the contents pane, you will need to run the `🛠️ XS River Position` tool to view them again.
...
8.	Repeat `🛠️ XS River Position` tool for the older gdb.

### 	XS Points 
1.	In the FluvialGeomorph toolbox, select `🛠️ XSpoints`.
2.	Enter feature dataset for more recent geodatabase.
3.	Input `🗺️ XS` line feature class.
4.	Input `🗺️ dem_hydro` raster.
5.	Change `📊 dem_units` to `🧮 ft`.
6.	Input `🗺️ REM` raster for the `📊 REM_dem`.
7.	`📊 Station Distance` is `🧮 1`.
8.	Hit Run.
9.	Points have been created every foot along the XS's with an Elevation. This will provide a visual elevation profile for each XS.
10.	Repeat these steps for the older gdb.

### 	XS Point Classify 
1.	In the FluvialGeomorph toolbox, select `🛠️ XS Points Classify`.
2.	Enter feature dataset for the most recent year.
3.	Input `🗺️ XSpoints` feature class.
4.	Input `🗺️ channel` polygon feature class.
5.	Input `🗺️ floodplain` polygon feature class.
6.	Leave `📊 buffer distance` as `🧮 default`.
...
1.	In the FluvialGeomorph toolbox, open `🛠️ XS Dimensions, Level 1`.
2.	`🗺️ XS` line feature class are added into `📊 xs_line fc`.
...
a.	In the FluvialGeomorph toolbox, open `🛠️ Join From CSV` tool located in the `🧰 Data Management` toolset.
b.	Feature_dataset will be the `🗺️ features` point feature class of the most recent gdb.
c.	Fc will be the `🗺️ XS` line feature class.
d.	Choose `🧮 Seq` in the dropdown for the `📊 fc_field`.
e.	The `📊 csv_file` will be the `🧮 .csv` file created from the `🛠️ XS Dimensions, Level 1` Tool.
...
1.	In the FluvialGeomorph toolbox, open the `🛠️ Report – L1` tool located in the ` 🧰 Reports` toolset.
...
3.	`📊 flowline_fc` will be the `🗺️ flowline` feature class from the recent year.
4.	`📊 XS_dimensions_fc` will be the line feature class created during the `🛠️ Join From CSV` tool.
5.	`📊 flowline_points_1` will be the `🗺️ flowlinepoints` feature classfrom the most recent year.
    i.	`📊 flowline_points_2` will be the `🗺️ flowlinepoints` feature class from the older year.
6.	`📊 XS_points_1` will be the `🗺️ XSpoints` point feature class created from the  XS Points tool. Use the data from the most recent year.
    i.	`📊 XS_points_2` will be the `🗺️ XSpoints` feature class for the older year.
...
8.	`📊 Features_fc` will need the `🗺️ features` point feature class that was created to indicate roadways, site improvements, or other bodies of water.
...
11.	`📊 Dem` will need the `🗺️ dem_hydro` raster.
12.	Check the box for: show_xs_map.
13.	Change profile_units to feet.
14.	Check boxes for aerial and elevation.
15.	`📊 XS_label_freq` and `📊 exaggeration` should already be set to `🧮 10` and `🧮 1.2`, respectively.
