...
Level 2 Estimate Bankfull is to estimate the REMed bankfull elevation for the base year for each reach. This is accomplished by identifying and mapping riffle cross sections and roughly estimate an initial bankfull elevation for the base year for each reach.
...
1.	Copy the `🗺️ XS` line feature class in the most recent database and rename one `🗺️ XS_Layout_riffles_ch` feature class (Ex. xs_300_150_riffels_ch).
...
1.	In the catalog pane, go to the `🧰 FluvialGeomorph toolbox`. Expand tools and select `🛠️ XS Resequence`.
2.  Feature_dataset for the most recent year.
3.	Xs_fc for the `🗺️ XS_Layout_riffles_ch`  feature class. 
...
1.	Copy the `🗺️ XS_Layout_riffles_ch` line feature class to the current year feature dataset and rename it `🗺️ XS_Layout_riffles_fp`.
2.	Edit the `🗺️ XS_Layout_riffles_fp` line  feature class to the full width of the floodplain polygon. The `🗺️ XS_Layout_riffles_fp` line feature class must line up with the `🗺️ XS_Layout_riffles_ch` line feature class. 
3.	Save edits.
 
### 	Calculate Cross Section Watershed Area – for the channel and floodplain riffles
1.	In the fluvialgeomorph folder, expand layers and add `🗺️ NHDPlusFac_National.tif` raster.
...
5.	Go to FluvialGeomorph toolbox open `🛠️ XS Water Area tool`.
...
9.	Once it is done running, open attribute table and see if anything in the `📊 watershedareaSqMile` is off.
    i.	Some of the cross sections `📊 Watershed_Area_SqMile` may not follow the trend. If this is the case, manually enter in the values to fill in the gaps. If most of the cross sections are missing these or display incorrect values, rerun `🛠️ Watershed Area Tool` and increase the `📊 snap distance`.
...
1.	Import the finished `🗺️ XS_Layout_riffles_ch` and `🗺️ XS_Layout_riffles_fp` feature classes to the earlier year feature dataset. 

### 	XS River Position 
1.	In the FluvialGeomorph toolbox, select `🛠️ XS River Position`.
2.	Enter feature dataset for more recent geodatabase.
3.	Input `🗺️ Riffle_XS` line feature class.
4.	Input `🗺️ Flowlinepoints` feature class.
5.	Hit Run. Complete for both the `🗺️ channel_XS` and `🗺️floodplain_XS`  feature classes.
...
8.	Hit Run. Complete for both the `🗺️ channel_XS` and `🗺️ floodplain_XS` feature classes.
...
### 	XS Point Classify 
1.	In the FluvialGeomorph toolbox, select `🛠️ XS Point Classify`.
...
7.	Leave `📊 buffer distance` as `🧮 default`.
8.	Repeat for earlier event `🗺️ XSpoints`  feature class.
...
2.	`🗺️ XS` line  feature class for the newest event are added into `📊 xs_line fc`.
...
2.	`🗺️ XS` dimension level 1 for the riffles into the `📊 xs_line fc`. Ex. xs_300_150_riffels_ch_103_5_dims_L1
3.	`🗺️ XS_points_channel_riffles` are added to the `📊 XS_point_fc`. Ex. xs_300_150_riffels_ch_103_5.
4.	`📊 Bankfull_elevation` is from the `📊 channel depth` of the `🗺️ bank_rawchannel`  feature class. Ex. 103.5
...
###	Join from CSV 
1.	In the FluvialGeomorph toolbox, open `🛠️ Join From CSV` located in the `🧰 Data Management toolset`.
2.	`🗺️ Features` dataset will be the feature dataset of the most recent gdb.
3.	Fc will be the `🗺️ XSchannelriffles _dims_L1` line feature class.
4.	Choose `🧮 Seq` in the dropdown for the `📊 fc_field`.
5.	The csv_file will be the `🗺️ .csv` file created from the ``XS Dimensions Level 2 Tool.
6.	`📊 Csv_field` will match the `📊 fc_field` so it should also be `🧮 Seq`.
7.	Hit Run.
8.	A new line feature class ending with `🗺️ *dims_L2` will be added to the feature dataset.

###	Generate Level L2 Estimate Bankfull Report
1.	In the FluvialGeomorph toolbox, open the `🛠️ Report – L2 Estimate Bankfull` located in the `🧰 Reports toolset`.
2.	`📊 Stream` will be the`🧮 ReachName` which was created when making the flowline. Copy and paste the `🧮 ReachName` from `🗺️ XS` feature class attribute table.
3.	`📊 Flowline_fc` will be the `🗺️ flowline` feature class from the recent survey event.
4.	`📊 Xs_dimensions_fc` will be the `🗺️ XS` line feature class created during the `🛠️ Join From CSV tool`.
5.	`📊 Xs_point_ch_1` will be the `🗺️ flowlinepoints` feature class from the most recent survey event.
    i.	`📊 Xs_point_ch_2` will be the `🗺️ flowlinepoints` feature class from the older survey event.
6.	`📊 Xs_points_fp_1` will be the `🗺️ XSpoints` feature class created from the `🛠️ XS Points tool`. Use the data from the most recent survey event.
    i.	`📊 Xs_points_fp_2` will be the `🗺️ XSpoints` feature class for the older survey event.
7.	`📊 Survey_name_1` will be the `🗺️ survey event` feature class of the most recent data (ex: 2022).
    i.	`📊 Survey_name_2` will be the `🗺️ survey event` feature class of the older data (ex: 2012).
8.	`📊 Features_fc` will need the `🗺️ features` point feature class that was created to indicate roadways, site improvements, or other bodies of water.
    i.	`📊 Channel_fc` will be the lower number ` 🗺️ bank_raw layer` feature class that represents the channel.
9.	`📊 Floodplain_fc` will be the higher number ` 🗺️ bank_raw layer` feature class that represents the floodplain,
10.	`📊 Dem` will need the `🗺️ dem_hydro`.
...
14.	`📊 By_elevation` must be a number divisible by the `📊 bf_estimate`. It normally ranges from 0.1-2.
