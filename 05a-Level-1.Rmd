...
The purpose of this step is to create a new study area geodatabase to store the project study area data. This step will need to be repeated for each [LiDAR survey](#units_of_analysis) that exists for this project study area. For example:
...
## Derive Flowline
The purpose of this stage is to create a new site geodatabase and new reach geodatabases for each reach and derive each reach's flowline. 

### Create the site geodatabase
The purpose of this step is to create a new site geodatabase and populate it with initial data. In the example above, the Papillion Creek project study area could be subdivided into a specific site called Cole Creek. 

* Cole_Creek_2016.gdb
* Cole_Creek_2010.gdb
* Cole_Creek_2006.gdb

* In the [study area folder](#folder_structure), create a new site folder named for the site. 
...
### Create the Flowline
The purpose of this step is to derive the reach flowline. The `05 - Flowline` tool converts a `stream_network` feature into a `flowline` FC. It smooths the geometry and converts it into a route. 
...
* Back in the site geodatabase, select the feature in the `stream_network` feature class representing the current study reach. Use the "Data | Export Features" function to export the selected reach feature to the new reach geodatabase. Name the exported feature class `stream_network`. 
* Ensure this new reach geodatabase version of `stream_network` contains only one feature representing the current reach. 
